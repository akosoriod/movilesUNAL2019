package co.edu.aiteaching.llamadas;
        ...
        try {
            ...
            if (info.equals("info")){
                for(int i=0;i<jsonArray.length();i++) {
                    ...
                    datos = new ArrayList<String>();
                    String data = jsonArray.getJSONObject(i).getString("ciudad");
                    datos.add(data);
                    data = jsonArray.getJSONObject(i).getString("a_o");
                    datos.add(data);
                    data = jsonArray.getJSONObject(i).getString("operador");
                    datos.add(data);
                    data = jsonArray.getJSONObject(i).getString("valor");
                    datos.add(data);
